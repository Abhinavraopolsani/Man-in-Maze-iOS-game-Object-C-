...
-(void)flipCardAtIndex:(NSUInteger) index
{
    ...
    if(card && !card.unplayable)
    {
        ...
        if(!card.isFaceup)
        {
            ...
            for(Card *otherCard in self.cards)
            {
                if(otherCard.isFaceup && !otherCard.isUnplayable)
                {
                    ...
                    if([otherCardsToMatch count] == self.numberOfMatchedCardRequired -1)
                    { // only try to match when reach number of cards required
                        int matchScore = [card match:otherCardsToMatch];
                        self.del.flipcount+=2;
                        if(matchScore)
                        {
                            card.unplayable = YES;
        ...
        if(self.pointsEarnInLastOperation == FLIP_PENALTY || self.pointsEarnInLastOperation==0)
        { // if not to reach com
